...
export class Mapbox extends MapboxCommon {
    ...
    constructor() {
        ...
        // pause
        application.android.on(application.AndroidApplication.activityPausedEvent, (args) => {
        ...
        // stop
        application.android.on(application.AndroidApplication.activityStoppedEvent, (args) => {
        ...
        // destroy
        application.android.on(application.AndroidApplication.activityDestroyedEvent, (args) => {
        ...
        // savestate
        application.android.on(application.AndroidApplication.saveActivityStateEvent, (args) => {
            console.log(
                "Mapbox::constructor: saveActivityStateEvent Event: " +
                    args.eventName +
                    ", Activity: " +
                    args.activity +
                    ", Bundle: " +
        ...
        application.android.on(application.AndroidApplication.activityResultEvent, (args) => {
            console.log(
                "Mapbox::constructor: activityResultEvent Event: " +
                    args.eventName +
                    ", Activity: " +
                    args.activity +
                    ", requestCode: " +
    ...
    handleLineClickEvent(clickOverlay) {
        let lineEntry = this.lines.find((entry) => {
            console.log(
                "Mapbox:handleLineClickEvent(): checking lineEntry clickOverlay id '" +
                    entry.clickOverlay +
                    "' against clickOverlay '" +
                    clickOverlay +
                    "'"
            );
    ...
    checkForCircleClickEvent(point) {
        ...
        for (let i = 0; i < this.circles.length; i++) {
            ...
            if (
                ...
                console.log(
                    "Mapbox:checkForCircleClickEvent() Point is in circle with id '" +
                        this.circles[i].id +
                        "' invoking callbacks, if any. Callback list is:",
                    this.eventCallbacks
                );
                for (let x = 0; x < this.eventCallbacks["click"].length; x++) {
                    let entry = this.eventCallbacks["click"][x];
            ...
            try {
                ...
                this.onDidFinishLoadingStyleListener = new com.mapbox.mapboxsdk.maps.MapView.OnDidFinishLoadingStyleListener({
                    onDidFinishLoadingStyle: (style) => {
                        ...
                        this.lineManager = new com.mapbox.mapboxsdk.plugins.annotation.LineManager(
                            this._mapboxViewInstance,
                            this._mapboxMapInstance,
                            this._mapboxMapInstance.getStyle()
                        );
            ...
            for (let m in updatedMarkers) {
                ...
                if (marker.icon) {
                    ...
                    if (marker.icon.startsWith("res://")) {
                        ...
                        if (identifier === 0) {
                            console.log(
                                `No icon found for this device density for icon ' ${marker.icon}'. Falling back to the default icon.`
                            );
                        } else {
                            markerOptions.setIcon(iconFactory.fromResource(identifier));
                        }
                    ...
                    for (let m in this._markers) {
                        ...
                        if (marker.id === _marker.id) {
                            ...
                            if (newSettings.lat && newSettings.lng) {
                                _marker.lat = newSettings.lat;
                                _marker.lng = newSettings.lng;
                                _marker.android.setPosition(
                                    new com.mapbox.mapboxsdk.geometry.LatLng(parseFloat(newSettings.lat), parseFloat(newSettings.lng))
                                );
                            }
                            if (newSettings.selected) {
        ...
        return new Promise((resolve, reject) => {
            try {
                ...
                if (this._mapboxMapInstance.queryRenderedFeatures) {
                    const features /* List<Feature> */ = this._mapboxMapInstance.queryRenderedFeatures(
                        screenLocation,
                        null,
                        options.layerIds
                    );
        ...
        return new Promise((resolve, reject) => {
            try {
                ...
                const cameraPositionBuilder = new com.mapbox.mapboxsdk.camera.CameraPosition.Builder(
                    this._mapboxMapInstance.getCameraPosition()
            ...
            try {
                ...
                fillExtrusionLayer.setFilter(
                    com.mapbox.mapboxsdk.style.expressions.Expression.eq(
                        com.mapbox.mapboxsdk.style.expressions.Expression.get("extrude"),
                        "true"
                    )
                );
                ...
                fillExtrusionLayer.setProperties(
                    ...
                    com.mapbox.mapboxsdk.style.layers.PropertyFactory.fillExtrusionHeight(
                        com.mapbox.mapboxsdk.style.functions.Function.property(
                            "height",
                            new com.mapbox.mapboxsdk.style.functions.stops.IdentityStops()
                        )
                    ),
            ...
            try {
                ...
                if (typeof style.paint == "undefined") {
                    ...
                    if (style.paint["line-opacity"]) {
                        lineProperties.push(
                            com.mapbox.mapboxsdk.style.layers.PropertyFactory.lineOpacity(new java.lang.Float(style.paint["line-opacity"]))
                        );
                    }
                    ...
                    if (style.paint["line-width"]) {
                        lineProperties.push(
                            com.mapbox.mapboxsdk.style.layers.PropertyFactory.lineWidth(new java.lang.Float(style.paint["line-width"]))
                        );
                    }
                    // line dash array
                ...
                if (typeof style.layout == "undefined") {
                    lineProperties = [
                        com.mapbox.mapboxsdk.style.layers.PropertyFactory.lineCap(
                            com.mapbox.mapboxsdk.style.layers.PropertyFactory.LINE_CAP_ROUND
                        ),
                        com.mapbox.mapboxsdk.style.layers.PropertyFactory.lineJoin(
                            com.mapbox.mapboxsdk.style.layers.PropertyFactory.LINE_JOIN_ROUND
                        ),
                    ];
                } else {
                    // line cap
                ...
                if (typeof style.paint == "undefined") {
                        ...
                        com.mapbox.mapboxsdk.style.layers.PropertyFactory.circleRadius(
                            com.mapbox.mapboxsdk.style.expressions.Expression.interpolate(
                                ...
                                [
                                    ...
                                    com.mapbox.mapboxsdk.style.expressions.Expression.stop(
                                        new java.lang.Float(20),
                                        new java.lang.Float(6000)
                                    ),
                                ]
                            )
                        ),
                    ...
                    if (style.paint["circle-opacity"]) {
                        circleProperties.push(
                            com.mapbox.mapboxsdk.style.layers.PropertyFactory.circleOpacity(
                                new java.lang.Float(style.paint["circle-opacity"])
                            )
                        );
                    }
                    ...
                    if (style.paint["circle-stroke-width"]) {
                        circleProperties.push(
                            com.mapbox.mapboxsdk.style.layers.PropertyFactory.circleStrokeWidth(
                                new java.lang.Float(style.paint["circle-stroke-width"])
                            )
                        );
                    }
                    // stroke color
                    if (style.paint["circle-stroke-color"]) {
                        circleProperties.push(
                            com.mapbox.mapboxsdk.style.layers.PropertyFactory.circleStrokeColor(style.paint["circle-stroke-color"])
                        );
                    }
                    if (!style.paint["circle-radius"]) {
                        ...
                        if (typeof style.paint["circle-radius"] == "number") {
                            circleProperties.push(
                                com.mapbox.mapboxsdk.style.layers.PropertyFactory.circleRadius(new java.lang.Float(style.paint.radius))
                            );
                        } else {
                            ...
                            for (let i = 0; i < style.paint["circle-radius"].stops.length; i++) {
                                ...
                                stopArgs.push(
                                    com.mapbox.mapboxsdk.style.expressions.Expression.stop(
                                        new java.lang.Float(stop[0]),
                                        new java.lang.Float(stop[1])
                                    )
                                );
                            }
                            let base = 2;
        ...
        return new Promise((resolve, reject) => {
            try {
                ...
                unclustered.setFilter(
                    com.mapbox.mapboxsdk.style.expressions.Expression.neq(
                        com.mapbox.mapboxsdk.style.expressions.Expression.get("cluster"),
                        true
                    )
                );
                ...
                for (let i = 0; i < layers.length; i++) {
                    ...
                    const pointCount = com.mapbox.mapboxsdk.style.expressions.Expression.toNumber(
                        com.mapbox.mapboxsdk.style.expressions.Expression.get("point_count")
                    );
                    circles.setFilter(
                        i === 0
                            ? com.mapbox.mapboxsdk.style.expressions.Expression.gte(
                                  pointCount,
                                  com.mapbox.mapboxsdk.style.expressions.Expression.literal(java.lang.Integer.valueOf(layers[i][0]))
                              )
    ...
    _fineLocationPermissionGranted() {
        ...
        if (!hasPermission) {
            hasPermission = com.mapbox.android.core.permissions.PermissionsManager.areLocationPermissionsGranted(
    ...
    showUserLocationMarker(options, nativeMap) {
        return new Promise((resolve, reject) => {
            try {
                ...
                let activationOptionsBuilder = com.mapbox.mapboxsdk.location.LocationComponentActivationOptions.builder(
                    application.android.context,
                    this._mapboxMapInstance.getStyle()
                );
    ...
    changeUserLocationMarkerMode(renderModeString, cameraModeString, nativeMap) {
        return new Promise((resolve, reject) => {
            try {
                ...
                console.log(
                    "Mapbox::changeUserLocationMarkerMode(): changing renderMode to '" +
                        renderModeString +
                        "' cameraMode '" +
                        cameraModeString +
                        "'"
